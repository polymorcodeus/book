...
# book installer script
# Downloads and installs the latest release of book
...
install_book() {
    ...
    echo -e "${BLUE}Installing book...${NC}"
    ...
    # Download URL
    local filename="book_${platform}.tar.gz"
    ...
    echo -e "${GREEN}book installed successfully!${NC}"
    echo -e "${GREEN}Run 'book --help' to get started.${NC}"

    # Test the installation
    if command -v book >/dev/null 2>&1; then
        echo -e "${GREEN}Installed version: $(book --version)${NC}"
    fi
}
